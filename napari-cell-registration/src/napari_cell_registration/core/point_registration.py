...
def estimate_robust_transform(
    ...
    *,
    ...
    candidates: list[tuple[str, type[AffineTransform], int, float]] = []
    if prefer_affine and len(pts_fixed) >= 4:
        candidates.append(("affine", AffineTransform, 4, residual_threshold))
    candidates.append(("similarity", SimilarityTransform, 3, similarity_threshold))

    best: RobustTransformResult | None = None
    for method_name, model_cls, min_samples, threshold in candidates:
        try:
        ...
        try:
            refit = estimate_transform(method_name, pts_moving[inliers], pts_fixed[inliers])
...
def _central_crop_pair(
    ...
    if 0.0 < crop_ratio < 1.0:
        ...
        y0 = max((h - crop_h) // 2, 0)
        x0 = max((w - crop_w) // 2, 0)
        ref = ref[y0 : y0 + crop_h, x0 : x0 + crop_w]
        mov = mov[y0 : y0 + crop_h, x0 : x0 + crop_w]

    return ref, mov


def refine_transform_with_phase_correlation(
    moving_image: np.ndarray,
    ...
    *,
    ...
    target_shape = output_shape or reference_image.shape[:2]
    transform = _as_affine_transform(initial)
    total_shift_yx = np.zeros(2, dtype=float)
    reference_gray = _to_registration_gray(reference_image)

    for _ in range(max_iterations):
        registered = warp_image_with_transform(moving_image, transform, target_shape, order=1)
        moving_gray = _to_registration_gray(registered)
        ref_crop, mov_crop = _central_crop_pair(reference_gray, moving_gray, crop_ratio)
        ...
        if shift_mag < min_improvement or shift_mag > max_shift:
            break

        delta = AffineTransform(translation=(float(shift_yx[1]), float(shift_yx[0])))
        transform = compose_affine_transforms(transform, delta)
        total_shift_yx += shift_yx

    return transform, total_shift_yx
...
def estimate_initial_transform(
    ...
    if use_ransac:
        robust = estimate_robust_transform(
            pts_r1,
            pts_r2,
            prefer_affine=(method == "affine"),
            residual_threshold=5.0,
            max_trials=100,
            min_inliers=4 if method == "affine" else 3,
