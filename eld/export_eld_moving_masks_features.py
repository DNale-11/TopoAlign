"""Segment ELD moving images and export masks plus cell features."""
...
MOVING_IMAGE_DIR = ELD_DIR / "moving"
MOVING_MASK_DIR = ELD_DIR / "moving mask"
MOVING_FEATURE_DIR = ELD_DIR / "moving feature"
...
def _export_one(image_path: Path, mask_path: Path, feature_path: Path, segmenter: CellposeSegmenter) -> None:
...
def main() -> None:
    if not MOVING_IMAGE_DIR.is_dir():
        raise FileNotFoundError(f"Moving image directory not found: {MOVING_IMAGE_DIR}")

    moving_files = sorted(MOVING_IMAGE_DIR.glob("*.tif"))
    if not moving_files:
        raise FileNotFoundError(f"No TIFF images found in {MOVING_IMAGE_DIR}")
    ...
    for image_path in moving_files:
        mask_path = MOVING_MASK_DIR / image_path.name
        feature_path = MOVING_FEATURE_DIR / f"{image_path.stem}.csv"
        _export_one(image_path, mask_path, feature_path, segmenter)

    print(f"Exported moving images: {len(moving_files)}")
    print(f"Mask output: {MOVING_MASK_DIR}")
    print(f"Feature output: {MOVING_FEATURE_DIR}")
