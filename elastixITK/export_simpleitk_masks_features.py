"""Export masks and feature CSVs for SimpleITK registration outputs."""
...
SIMPLEITK_DIR = Path(__file__).resolve().parent
FIXED_DIR = SIMPLEITK_DIR / "fixed_images"
REGISTERED_DIR = SIMPLEITK_DIR / "registered"
FIXED_PATTERN = re.compile(r"C-(\d+)_B_raw_fixed\.tif$", re.IGNORECASE)
MOVING_PATTERN = re.compile(r"C-(\d+)_([A-Z])_to_B_registered\.tif$", re.IGNORECASE)


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export SimpleITK masks and feature CSVs.")
    parser.add_argument(
        "--params-json",
        type=Path,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=SIMPLEITK_DIR / "masks_features",
...
def _find_fixed_image(group: int) -> Path:
    folder = FIXED_DIR / f"C-{group}"
    if not folder.is_dir():
        raise FileNotFoundError(f"Fixed image folder not found: {folder}")
...
def _find_registered_images(group: int) -> dict[str, Path]:
    folder = REGISTERED_DIR / f"C-{group}"
    if not folder.is_dir():
        raise FileNotFoundError(f"Registered image folder not found: {folder}")
...
def main() -> None:
    ...
    for group in sample_groups:
        ...
        moving_images = _find_registered_images(group)
        for round_name in rounds:
            if round_name == "B":
                continue
            moving_path = moving_images.get(round_name)
            if moving_path is None:
                print(f"[G{group}] missing registered image for round {round_name}, skipped")
                continue
